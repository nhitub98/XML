...
class khachhang{
    ...
    public function Add()
    {
        ...
        $rootTag=$root->getElementsByTagName("listcustomer")->item(0);
...
$kq=$xpath->query("/listcustomer/customer[@id=$id]"); 
foreach ($kq as $node)
{
//Tao node moi
$infoTag=$root->createElement("customer");
...
$root->load('khachhang.xml');
$xpath=new DOMXPATH($root);
foreach ($xpath->query("/listcustomer/customer[@id=$id]") as $node)
{
